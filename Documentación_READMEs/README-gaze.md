...
## Gaze-related Material
#### Last modification: 24/02/2025

Thanks for downloading the DMD! :)

This dataset is public and free for academic use. For easier access, the data been **divided** and organized by its primary applications, offered through **three different downloading services**. This repository specifically contains data related to **gaze estimation**. This is the [*Drowsiness*](https://opendatasets.vicomtech.org/di21-dmd-dataset-drowsiness/99e2fbd7) downloading form. Distraction downloading form is still work in progress.

Gaze estimation data contains videos of drivers staring at 9 specific regions inside the car. Also, there is material for hands-wheel interation, which shows the drivers with differents hands positions.
...
The defined levels describe temporal actions or events which occur when the driver is performing some gaze or hands-related actions. To annotate temporal actions, we defined 3 levels of annotations for gaze and 4 for hands. Basically, they are annotations that can simultaneously be present and describe one frame. Each level of annotation has its own set of labels. Within each level, the labels are mutually exclusive, meaning that, for each level a maximum of one label is allowed.
...
The hands-related annotation levels are:
 - Occlusion
    - Face occlusion	
    - Body occlusion
    - Hands occlusion
...
To perform the temporal action annotation of the DMD we have defined an **annotation criterion** that has to be followed by all annotators to guarantee consistent annotation production. Here, there is the document for [gaze annotations](https://github.com/Vicomtech/DMD-Driver-Monitoring-Dataset/wiki/DMD-gaze-related-action-annotation-criteria) and [hands annotations]https://github.com/Vicomtech/DMD-Driver-Monitoring-Dataset/wiki/DMD-hands-related-action-annotation-criteria).
...
![alt text](https://raw.githubusercontent.com/Vicomtech/DMD-Driver-Monitoring-Dataset/refs/heads/master/docs/readme-assets/gazeRegions.png)
